...
import core.spec.IPluginDescribable;
import core.spec.IPluginPropertyHolder;
import core.spec.PluginDescriptionDeclaration;
import core.spec.PluginPropertyDeclaration;
import core.util.Assert;
import core.util.XMLPublicIDUtil;

/**
 * Scanner for building Tapestry Specs - this is the base class for Component,
 * Library, and Application scanners. This class handles some setup and
 * validates the document by default.
 * 
...
public abstract class SpecificationScanner extends AbstractDOMScanner {
	...
	protected Object beforeScan() throws ScannerException {
		...
		if (fPublicId == null)
			throw new ScannerException(CoreMessages
					.format(XMLPublicIDUtil.SPEC_DTD_ERROR_KEY), false,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see core.scanning.AbstractScanner#doScan(java.lang.Object,
	 *      java.lang.Object)
	 */
...

	{
		public Object convert(String value) throws ScannerException {
			...
			if (result == null || !(result instanceof Boolean))
				throw new ScannerException(ParseMessages
						.failConvertBoolean(value), false,
	...
	public static class IntConverter implements IConverter {
		public Object convert(String value) throws ScannerException {
			try {
				...
				throw new ScannerException(ParseMessages.failConvertInt(value),
						ex, false, IProblem.TAP_FAILED_TO_CONVERT_TO_INT);
			}
		}
	}
		...
		public Object convert(String value) throws ScannerException {
			try {
				...
				throw new ScannerException(
						ParseMessages.failConvertLong(value), ex, false,
	...
	public static class DoubleConverter implements IConverter {
		public Object convert(String value) throws ScannerException {
			try {
				...
				throw new ScannerException(ParseMessages
						.failConvertDouble(value), ex, false,
	...
	/**
	 * We can share a single map for all the XML attribute to object
	 * conversions, since the keys are unique.
	 */
	...
	protected boolean scanMeta(IPluginPropertyHolder holder, Node node)
	...
	private boolean scanProperty(IPluginPropertyHolder holder, Node node,
			String keyAttribute, String valueAttribute) throws ScannerException {
		...
		if (holder.getProperty(name) != null) {
			addProblem(IProblem.WARNING, getAttributeSourceLocation(node,
					keyAttribute), "duplicate definition of property: " + name,
					false, IProblem.SPINDLE_DUPLICATE_PROPERTY_ID);
		}
		...
		PluginPropertyDeclaration declaration = new PluginPropertyDeclaration(
				name, value);
		declaration.setLocation(getSourceLocationInfo(node));
		declaration.setValueIsFromAttribute(result == null ? false
				: result.fromAttribute);

		if (value != null && value.trim().length() == 0) {
			addProblem(IProblem.WARNING,
					result.fromAttribute ? getAttributeSourceLocation(node,
							valueAttribute) : getBestGuessSourceLocation(node,
							true), "missing value of property: " + name, false,
	...
	protected void allowMeta(IPropertyHolder holder, Node node)
			throws ScannerException {
	...
	/**
	 * Used with many elements that allow a value to be specified as either an
	 * attribute, or as wrapped character data. This handles that case, and
	 * makes it an error to specify both.
	 */

	protected ExtendedAttributeResult getExtendedAttribute(Node node,
			String attributeName, boolean required) throws ScannerException {
		...
		if (!nullAttributeValue && !nullBodyValue)
			throw new ScannerException(ParseMessages.noAttributeAndBody(
					attributeName, node.getNodeName()),
					getNodeBodySourceLocation(node), false,
		...
		if (required && nullAttributeValue && nullBodyValue)
			throw new ScannerException(ParseMessages.requiredExtendedAttribute(
					...
					getNodeStartSourceLocation(node), false,
	...
	protected boolean scanDescription(IPluginDescribable describable, Node node) {
		...
		PluginDescriptionDeclaration declaration = new PluginDescriptionDeclaration(
				null, value, getSourceLocationInfo(node));
