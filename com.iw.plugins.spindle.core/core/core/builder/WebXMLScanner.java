...
import core.scanning.AbstractDOMScanner;
...
public abstract class WebXMLScanner extends AbstractDOMScanner {
	...
	public WebAppDescriptor scanWebAppDescriptor(IDOMModel source)
	...
	protected void checkApplicationLocation(ICoreResource location)
		...
		if (!location.exists())
			throw new ScannerException(CoreMessages.format(
					...
					location == null ? "no location found" : location
		...
		if (extension == null
				|| !extension
						.equals(AbstractBuildInfrastructure.APPLICATION_EXTENSION))
			throw new ScannerException(CoreMessages.format(
					"web-xml-wrong-file-extension", location.toString()),
					false, IProblem.NOT_QUICK_FIXABLE);

	}

	protected void checkApplicationServletPathParam(String value,
			ServletInfo currentInfo, ISourceLocation location) {
		if (currentInfo.isServletSubclass
				&& currentInfo.applicationSpecLocation != null) {
		...
		if (!value.endsWith(".application")) {
			addProblem(IProblem.ERROR, location, CoreMessages.format(
					"web-xml-wrong-file-extension", value), false,
		...
		if (ws_location == null) {
			addProblem(IProblem.ERROR, location, CoreMessages.format(
					"web-xml-ignore-application-path-not-found", value), false,
					IProblem.NOT_QUICK_FIXABLE);
			return;
		}
	...
	protected boolean checkJavaSubclassOfImplements(IJavaType superclass,
			IJavaType candidate, ISourceLocation location) {
		if (superclass.equals(candidate))
			return true;

		try {
			if (candidate.isInterface() || candidate.isAnnotation()) {
				addProblem(IProblem.ERROR, location,
						"web-xml-must-be-class-not-interface", false,
	...
	protected IJavaType checkJavaType(String className, ISourceLocation location) {
		...
		if (found == null)
			addProblem(IProblem.ERROR, location, DefaultTapestryMessages
					.format("unable-to-resolve-class", className), false,
					IProblem.NOT_QUICK_FIXABLE);
	...
	protected void doScan() {
		...
		for (Node node = rootNode.getFirstChild(); node != null; node = node
				.getNextSibling()) {
		...
		descriptor.setServletInfos((ServletInfo[]) infos
				.toArray(new ServletInfo[] {}));
		descriptor.setContextParameters(contextParameters);
	}

	private void extractContextParameter(Node contextParamNode,
		...
		for (Node node = contextParamNode.getFirstChild(); node != null; node = node
			...
			if (isElement(node, "param-name")) {
				...
				if (key == null) {
					addProblem(IProblem.ERROR, keyLoc, CoreMessages
							.format("web-xml-context-param-null-key"), false,
							IProblem.NOT_QUICK_FIXABLE);
					return;
				} else if (target.containsKey(key)) {
					addProblem(IProblem.ERROR, keyLoc, CoreMessages.format(
							"web-xml-context-param-duplicate-key", key), false,
							IProblem.NOT_QUICK_FIXABLE);
					return;
				}
			}
			...
			if (isElement(node, "param-value")) {
				...
				if (value == null)
					addProblem(IProblem.ERROR, valueLoc, CoreMessages
							.format("web-xml-context-param-null-value"), false,
							IProblem.NOT_QUICK_FIXABLE);
			}
		}
	...
	/**
	 * Legacy (pre Tapestry 3.0) applications could only specify the location of
	 * the application specification in one way. By overriding the method
	 * getApplicationSpecificationPath() in ApplicationServlet.
	 * <p>
	 * This is Spindle's attempt to support this legacy method. Subclasses must
	 * override.
	 * 
	 * @param servletType
	 ...
	 */
	protected abstract String getApplicationPathFromServletSubclassOverride(
			IJavaType servletType) throws ScannerException;

	/**
	 * @param servletNode
	 *            a servlet xml node
	 * @return an instance of ServletInfo is the node contains a valid Tapestry
	 *         servlet
	 */
	protected ServletInfo extractServletInfo(Node servletNode) {
		...
		for (Node node = servletNode.getFirstChild(); node != null; node = node
			...
			if (isElement(node, "servlet-class")) {
				if (!extractServletClassAndApplicationSpecLocation(node,
						newInfo))
					return null;
				continue;
	...
	private boolean isTapestryServletOrSubclass(IJavaType candidate,
			ISourceLocation location) {
		if (candidate.equals(builder.tapestryServletType))
			return true;

		return checkJavaSubclassOfImplements(builder.tapestryServletType,
				candidate, location);
	}

	protected boolean extractServletInitParameter(Node initParamNode,
		...
		for (Node node = initParamNode.getFirstChild(); node != null; node = node
			...
			if (isElement(node, "param-name")) {
				...
				if (key == null) {
					addProblem(IProblem.ERROR, keyLoc, CoreMessages
							.format("web-xml-init-param-null-key"), false,
							IProblem.NOT_QUICK_FIXABLE);
					return false;
				} else if (currentInfo.parameters.containsKey(key)) {
					addProblem(IProblem.ERROR, keyLoc, CoreMessages.format(
							"web-xml-init-param-duplicate-key", key), false,
							IProblem.NOT_QUICK_FIXABLE);
					return false;
				}
			}
			...
			if (isElement(node, "param-value")) {
				...
				if (value == null)
					addProblem(IProblem.ERROR, valueLoc, CoreMessages
							.format("web-xml-init-param-null-value"), false,
							IProblem.NOT_QUICK_FIXABLE);
	...
	/**
	 * update the servlet info with the servlet class defined in the node and
	 * find the location of the application specification; logging any problems
	 * encountered
	 * 
	 * @param node
	 ...
	 * @return true iff
	 */
	protected boolean extractServletClassAndApplicationSpecLocation(Node node,
			ServletInfo newInfo) {
		...
		if (newInfo.classname == null) {
			String message = CoreMessages.format(
					"web-xml-servlet-null-classname", newInfo.name);
			addProblem(IProblem.WARNING, nodeLocation, message, false,
					IProblem.WEB_XML_MISSING_APPLICATION_SERVLET_CLASS);
			return false;
		} else {
			IJavaType servletType = checkJavaType(newInfo.classname,
					nodeLocation);
			if (servletType != null
					&& isTapestryServletOrSubclass(servletType, nodeLocation)) {
				newInfo.isServletSubclass = !builder.tapestryServletType
						.equals(servletType);
				...
				if (newInfo.isServletSubclass) {
					try {
						...
						try {
							...
							applicationSpecLocation = getApplicationLocation(
									newInfo, path);
						...
						addProblem(IProblem.ERROR, nodeLocation,
								e.getMessage(), false,
								IProblem.NOT_QUICK_FIXABLE);
					}
	 ...
	 * @param newInfo
	 *            the ServletInfo that carries the extracted information about
	 *            the servlet
	...
	protected boolean extractServletName(Node node, ServletInfo newInfo) {
		...
		ISourceLocation bestGuessSourceLocation = getBestGuessSourceLocation(
				node, true);
		if (servletName == null) {
			addProblem(IProblem.WARNING, bestGuessSourceLocation, CoreMessages
					.format("web-xml-servlet-has-null-name"), false,
					IProblem.NOT_QUICK_FIXABLE);

		} else {
			if (seenServletNames.contains(servletName)) {
				addProblem(IProblem.WARNING, bestGuessSourceLocation,
						CoreMessages.format("web-xml-servlet-duplicate-name",
								newInfo.name), false,
						IProblem.NOT_QUICK_FIXABLE);
			} else {
