...
public class DTDParser implements EntityExpansion
{
  ...
  public DTDParser(URL in, String publicId) throws IOException
  {
    ...
    defaultLocation = new URL(in.getProtocol(), in.getHost(), in.getPort(), file
        .substring(0, file.lastIndexOf('/') + 1));

    scanner = new Scanner(
        new BufferedReader(new InputStreamReader(in.openStream())),
        false,
        this);
    dtd = new DTD(publicId);
  }
   ...
   * invocation of parse does not try to guess the root element (for efficiency
   * reasons)
   */
   ...
   * 
   * @param guessRootElement If true, tells the parser to try to guess the root
   *          element of the document by process of elimination
   */
  ...
  protected void parseTopLevelElement() throws IOException
  {
    ...
    if (token.type == Scanner.LTQUES)
    {
      ...
      DTDProcessingInstruction instruct = new DTDProcessingInstruction(textBuffer
          .toString());
    ...
    } else
    {
      ...
      throw new DTDParseException(scanner.getUriId(), "Unexpected token: "
          + token.type.name + "(" + token.value + ")", scanner.getLineNumber(), scanner
          .getColumn());
    }
  ...
  protected Token expect(TokenType expected) throws IOException
  {
    ...
    if (token.type != expected)
    {
      if (token.value == null)
      {
        throw new DTDParseException(scanner.getUriId(), "Expected " + expected.name
            + " instead of " + token.type.name, scanner.getLineNumber(), scanner
            .getColumn());
      } else
      {
        throw new DTDParseException(scanner.getUriId(), "Expected " + expected.name
            + " instead of " + token.type.name + "(" + token.value + ")", scanner
  ...
  protected DTDItem parseCP() throws IOException
  {
    ...
    {
      throw new DTDParseException(scanner.getUriId(), "Found invalid token in sequence: "
  ...
  protected DTDNotationList parseNotationList() throws IOException
  {
    ...
    {
      throw new DTDParseException(scanner.getUriId(), "Invalid token in notation: "
          + token.type.name, scanner.getLineNumber(), scanner.getColumn());
    }
  ...
  protected DTDEnumeration parseEnumeration() throws IOException
  {
    ...
    for (;;)
    {
      ...
      {
        throw new DTDParseException(scanner.getUriId(), "Invalid token in enumeration: "
            + token.type.name, scanner.getLineNumber(), scanner.getColumn());
      }
  ...
  protected void parseEntity() throws IOException
  {
    ...
    {
      // 070501 MAW: If the entity already exists, create a dummy entity - this
      // way
