...
import com.iw.plugins.spindle.core.FullBuild.ServletInfo;
...
public class WebXMLProcessor {
  ...
  public FullBuild.ServletInfo[] getServletInformation(Element webxml) {
    ...
    return (FullBuild.ServletInfo[]) infos.toArray(new FullBuild.ServletInfo[infos.size()]);
  }
  ...
  protected boolean processServletClass(Node node, FullBuild.ServletInfo newInfo) {
    ...
    if (newInfo.classname == null) {
      ...
      builder.markError(
        ...
        builder.getBestGuessSourceLocation(node, false),
        message);
      if (TapestryBuilder.DEBUG) {
        System.out.println(message);
      }
    ...
    if (servletType == null) {
      builder.markError(
        ...
        builder.getBestGuessSourceLocation(node, true),
