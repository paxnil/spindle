 ...
 * 
 * It is assumed that the component we are scanning templates on behalf of has
...
public class TemplateScanner extends AbstractScanner
{
  ...
  protected void doScan(Object source, Object resultObject) throws ScannerException
  {
    ...
    } else
    {
      ...
      try
      {
        ...
        data = Files
            .readFileToString(in, fEncoding == null ? "UTF-8" : fEncoding)
            .toCharArray();
  ...
  private PluginContainedComponent createImplicitComponent(
      CoreOpenToken token,
      String id,
  ...
  private void validateImplicitComponent(
      PluginContainedComponent contained,
      CoreOpenToken token) throws ScannerException
  {
    ...
    if (!required.isEmpty())
    {
      addProblem(IProblem.ERROR, token.getEventInfo().getStartTagLocation(), TapestryCore
          .getTapestryString("PageLoader.required-parameter-not-bound", required
              .toString(), "'" + token.getComponentType() + "'"), false);
    }
    ...
    while (i.hasNext())
    {
      ...
      PluginBindingSpecification bspec = (PluginBindingSpecification) contained
          .getBinding(name);
      ISourceLocation location = getAttributeLocation(name, token
          .getEventInfo()
          .getAttributeMap());
      ...
      ISourceLocation location) throws ScannerException
  {
    ...
    boolean isAllowInformalParameters = containedSpecification
      ...
      ISourceLocation location)
  {
    ...
    if (isFormal)
    {
      ...
      if (!allowed)
        addProblem(IProblem.WARNING, location, "Parameter '" + bspec.getIdentifier()
            + "' of '" + containedSpecification.getSpecificationLocation().getName()
            + "' expects bindings to be of type '" + pType + "'", false);
    }
  ...
  private void validateExplicitComponent(
  ...
  /**
   * Check a template expression binding, look for errors related to reserved
   * and informal parameters.
   * 
   * <p>
   * It is an error to specify expression bindings in both the specification and
   * the template.
   *  
   */
    ...
    if (isFormal)
    {
      if (component.getBinding(name) != null)
        addProblem(IProblem.ERROR, location, TapestryCore.getTapestryString(
            ...
            fComponentSpec.getSpecificationLocation().getName()), false);
  ...
  /**
   * Check a string binding, look for errors related to reserved and informal
   * parameters.
   */
  ...
  /**
   * Check a static binding, look for errors related to reserved and informal
   * parameters.
   * 
   * <p>
      ...
      if (libraryId != null)
      {
        ...
        if (namespace == null)
          throw new ApplicationRuntimeException("Unable to resolve "
              + TapestryCore.getTapestryString("Namespace.nested-namespace", libraryId));
      }
