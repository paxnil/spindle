...
public class LibraryScanner extends SpecificationScanner
{
  ...
  protected void scanComponentType(ILibrarySpecification specification, Node node) throws ScannerException
  {
    ...
    PluginComponentTypeDeclaration declaration = new PluginComponentTypeDeclaration(
        type,
        path,
        getSourceLocationInfo(node));
  ...
  protected void scanConfigure(IExtensionSpecification spec, Node node) throws ScannerException
  {
    ...
    PluginExtensionConfiguration configuration = new PluginExtensionConfiguration(
        propertyName,
        value,
        type,
        getSourceLocationInfo(node));

    configuration.setDeclaredValueIsFromAttribute(result == null
        ? true : result.fromAttribute);
  ...
  protected void scanLibrary(ILibrarySpecification specification, Node node) throws ScannerException
  {
    ...
    PluginLibraryDeclaration declaration = new PluginLibraryDeclaration(
        id,
        specificationPath,
        getSourceLocationInfo(node));
  ...
  protected void scanPage(ILibrarySpecification specification, Node node) throws ScannerException
  {
    ...
    if (specification.getPageNames().contains(name))
      addProblem(IProblem.ERROR, getAttributeSourceLocation(node, "name"), TapestryCore
          .getTapestryString("LibrarySpecification.duplicate-page-name", name), false);

    String specificationPath = getAttribute(node, "specification-path");

    PluginPageDeclaration declaration = new PluginPageDeclaration(
        name,
        specificationPath,
        getSourceLocationInfo(node));
  ...
  protected void scanService(ILibrarySpecification spec, Node node) throws ScannerException
  {
    ...
    {
      addProblem(IProblem.ERROR, getAttributeSourceLocation(node, "name"), TapestryCore
          .getTapestryString("LibrarySpecification.duplicate-service-name", name), false);
    }

    PluginEngineServiceDeclaration declaration = new PluginEngineServiceDeclaration(
        name,
        className,
        getSourceLocationInfo(node));
