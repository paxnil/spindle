...
public abstract class SpecificationScanner extends AbstractScanner {
	...
	protected Object beforeScan(Object source) throws ScannerException {
		...
		if (fPublicId == null)
			throw new ScannerException(TapestryCore
					.getString(XMLUtil.SPEC_DTD_ERROR_KEY), false,
	...
	protected void checkTapestry_3_0_Document() throws IgnoreNodeException {
		if( fPublicIdCode != XMLUtil.DTD_3_0 )
			throw new IgnoreNodeException();
	}
	
	protected void checkTapestry_4_0_Document() throws IgnoreNodeException {
		if( fPublicIdCode != XMLUtil.DTD_4_0 )
			throw new IgnoreNodeException();
	}
		...
		public Object convert(String value) throws ScannerException {
			...
			if (result == null || !(result instanceof Boolean))
				throw new ScannerException(TapestryCore.getTapestryString(
						"SpecificationParser.fail-convert-boolean", value),
						false, IProblem.TAP_FAILED_TO_CONVERT_TO_BOOLEAN);
	...
	public static class IntConverter implements IConverter {
		public Object convert(String value) throws ScannerException {
			try {
				...
				throw new ScannerException(TapestryCore.getTapestryString(
						"SpecificationParser.fail-convert-int", value), ex,
						false, IProblem.TAP_FAILED_TO_CONVERT_TO_INT);
			}
		}
	}
	...
	public static class LongConverter implements IConverter {
		public Object convert(String value) throws ScannerException {
			try {
				...
				throw new ScannerException(TapestryCore.getTapestryString(
						"SpecificationParser.fail-convert-long", value), ex,
						false, IProblem.TAP_FAILED_TO_CONVERT_TO_LONG);
			}
		}
	}
	...
	public static class DoubleConverter implements IConverter {
		public Object convert(String value) throws ScannerException {
			try {
				...
				throw new ScannerException(TapestryCore.getTapestryString(
						"SpecificationParser.fail-convert-double", value), ex,
						false, IProblem.TAP_FAILED_TO_CONVERT_TO_DOUBLE);
			}
		}
	}
	...
	protected void scanProperty(IPluginPropertyHolder holder, Node node)
		...
		if (holder.getProperty(name) != null) {
			addProblem(IProblem.WARNING, getAttributeSourceLocation(node,
					"name"), "duplicate definition of property: " + name,
		...
		if (value != null && value.trim().length() == 0) {
			addProblem(IProblem.WARNING,
					result.fromAttribute ? getAttributeSourceLocation(node,
							"value") : getBestGuessSourceLocation(node, true),
					"missing value of property: " + name, false,
	...
	protected ExtendedAttributeResult getExtendedAttribute(Node node,
		...
		if (!nullAttributeValue && !nullBodyValue)
			throw new ScannerException(TapestryCore.getTapestryString(
					"SpecificationParser.no-attribute-and-body", attributeName,
					node.getNodeName()), getNodeBodySourceLocation(node),
					false, IProblem.EXTENDED_ATTRIBUTE_BOTH_VALUE_AND_BODY);

		if (required && nullAttributeValue && nullBodyValue)
			throw new ScannerException(TapestryCore.getTapestryString(
					"SpecificationParser.required-extended-attribute", node
