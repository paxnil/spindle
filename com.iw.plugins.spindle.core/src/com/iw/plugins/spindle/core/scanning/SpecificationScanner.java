 ...
 * Library, and Application scanners.
 * 
...
public abstract class SpecificationScanner extends AbstractScanner
{
  ...
  protected Object beforeScan(Object source) throws ScannerException
  {
    ...
    if (fPublicId == null || !checkPublicId())
    {
      throw new ScannerException(
          TapestryCore.getString(XMLUtil.SPEC_DTD_ERROR_KEY),
          false);
    }
    ...
    if (value != null && value.trim().length() == 0)
    {
      addProblem(
          ...
          result.fromAttribute
              ? getAttributeSourceLocation(node, "value") : getBestGuessSourceLocation(
                  node,
                  true),
   ...
   * attribute, or as wrapped character data. This handles that case, and makes
   * it an error to specify both.
   *  
   */

  protected ExtendedAttributeResult getExtendedAttribute(
      Node node,
