...
public class ElementSourceLocationInfo {
  ...
  */
  ...
  private void init(ElementXMLEventInfo eventInfo, ISourceLocationResolver resolver) {
    ...
    int line = startTag.getBeginLineNumber();
    int column = resolver.getColumnOffset(line, startTag.getBeginColumnNumber());

    startTagStartLine = sourceStartLine = line;
    startTagCharStart = sourceCharStart = column;

    line = startTag.getEndLineNumber();
