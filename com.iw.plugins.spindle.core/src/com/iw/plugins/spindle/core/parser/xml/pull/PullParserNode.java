...
/**
 * The implementation of org.w3c.dom.Node for the PullParser To create an elementNode...
 * PullParserNode.createElementNode(args); To create a Text node..
 * PullParserNode.createTextNode(args); Things that the creator method won't do for you: set the
 * first child and the siblings: you have to do this using the methods setFirstChild(),
 * setPreviousSibling(), and setNextSibling() You also need to call completed() when it appropriate
 * 
...
public class PullParserNode implements Node
{
     ...
     *            the parser in use
     * @param parentNode
     *            the parent, null if its the root node
     * @param elementName =
     *            Qname.rawname
     * @param attributes
     *            the XMLAttributes passed to startElement
     * @return
     */
    public static PullParserNode createElementNode(TapestryPullParser parser,
            PullParserNode parentNode, String elementName, XMLAttributes attributes)
    {

        return new PullParserNode(parser, elementName, PullParserNode.ELEMENT_NODE, parentNode,
                attributes);
    }
     ...
     * @param value
     *            The text returned by the characters() method in the parser
     * @return a new Text node
     * @throws IllegalArgumentException
     *             if parent node is null (text nodes can't be the root node)
     */
    public static PullParserNode createTextNode(TapestryPullParser parser,
    ...
    /**
     * Name will be: Node Type Name ---------------------- ELEMENT_NODE the name of the element (tag
     * name) TEXT_NODE "#text"
     */
    protected String fNodeName;

    /**
     * One of: ELEMENT_NODE TEXT_NODE There are many other kinds of Nodes declared in
     * org.w3c.dom.Node But we only care about the above two.
     */
    protected short fNodeType;

    /**
     * Value will be: Node Type Value ---------------------- ELEMENT_NODE null! TEXT_NODE The
