...
public class TemplateFinder
{
  ...
  /**
   * Filter a list of template locations. Use the extension defined in the
   * library spec, or the default (html) Exclude any localized names.
   * 
   * Klunky imlementation - will have to do until I figure out a regular
   * expression
   * 
   * @param locations
   * @param namespace
  ...
  private void readTemplatesFromAsset(
      ...
      IAssetSpecification templateAsset)
  {
    ...
    {
      addProblem(IProblem.WARNING, ((ISourceLocationInfo) templateAsset.getLocation())
          .getTagNameLocation(), "Spindle can't resolve templates from external assets");
      return;
    }
    ...
    if (type == AssetType.PRIVATE)
    {
      ...
      {
        addProblem(IProblem.ERROR, ((ISourceLocationInfo) templateAsset.getLocation())
            .getAttributeSourceLocation("resource-path"), TapestryCore.getTapestryString(
            "DefaultTemplateSource.unable-to-read-template",
            templatePath));
        return;
      }
  ...
  private void findPageTemplateInApplicationRoot(
      PluginComponentSpecification specification) throws CoreException
  {
  ...
  {
    IResourceWorkspaceLocation[] results = new IResourceWorkspaceLocation[fFindResults
        .size()];
