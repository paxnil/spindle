...
public class BuilderResourceVisitor implements IResourceVisitor
{
    ...
    public BuilderResourceVisitor(FullBuild build, ArrayList collector)
    {
    ...
    public boolean visit(IResource resource) throws CoreException
    {
        ...
        if (resource instanceof IFolder)
        {
            ...
            } else if (build.knownValidServlets != null)
            {
                ...
                if (resource.getParent().equals(tapestryProject.getWebContextFolder()) && knownServletNames.contains(folderName))
                {
                    collector.add(resource);
                    debug(resource, true);
                    return false;
