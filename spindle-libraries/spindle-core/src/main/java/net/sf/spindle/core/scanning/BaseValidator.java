...
public class BaseValidator implements IScannerValidator {
	...
	protected void fireTypeDependency(Resource dependant,
		...
		for (Iterator iter = fListeners.iterator(); iter.hasNext();) {
			IScannerValidatorListener listener = (IScannerValidatorListener) iter
					.next();
	...
	protected Pattern compilePattern(String pattern) {
		...
		try {
			return fPatternCompiler.compile(pattern,
	...
	public boolean validateExpression(String expression, int severity)
			throws ScannerException {
	...
	public void addProblem(int severity, ISourceLocation location,
		...
		if (fProblemCollector == null) {
			...
			fProblemCollector.addProblem(severity,
					(location == null ? DefaultSourceLocation : location),
					message, isTemporary, code);
		}
	}

	public boolean validatePattern(String value, String pattern,
			String errorKey, int severity, int code) throws ScannerException {
		return validatePattern(value, pattern, errorKey, severity,
				DefaultSourceLocation, code);
	}

	public boolean validatePattern(String value, String pattern,
			String errorKey, int severity, ISourceLocation location, int code)
			throws ScannerException {
		...
		if (value != null) {
			...
			if (!fMatcher.matches(value, compiled)) {
				addProblem(severity, location, ParseMessages.invalidAttribute(
						errorKey, value), false, code);
				return false;
			}
			return true;
		}
		addProblem(severity, location, ParseMessages.invalidAttribute(errorKey,
				"null value"), false, -1);
		return false;
	}
	...
	public boolean validateResource(Resource location, String relativePath,
			String errorKey, ISourceLocation source) throws ScannerException {
		return validateResourceLocation(location, relativePath, errorKey,
				source, false);
	}
	 ...
	 *      java.lang.String)
	 */
	public boolean validateResourceLocation(Resource location,
			String relativePath, String errorKey, ISourceLocation source,
			boolean accountForI18N) throws ScannerException {
		if (relativePath == null
				|| relativePath.startsWith(getDummyStringPrefix()))
			return false;
		...
		if (!relative.exists()) {
			...
			addProblem(IProblem.ERROR, source, CoreMessages.format(errorKey,
	...
	public Object validateTypeName(Resource dependant,
			String fullyQualifiedType, int severity) throws ScannerException {
		return validateTypeName(dependant, fullyQualifiedType, severity,
	...
	public Object validateTypeName(Resource dependant,
		...
		if (fullyQualifiedType == null) {
			addProblem(severity, location, DefaultTapestryMessages.format(
					"unable-to-resolve-class", "null value"), true, -1);
			return null;
		}
		...
		if (type == null) {
			addProblem(severity, location, DefaultTapestryMessages.format(
					"unable-to-resolve-class", fullyQualifiedType), true,
					IProblem.SPINDLE_MISSING_TYPE);
			return null;
		}
		return type;
	}
	 ...
	 */
	public void validateBindingReference(int severity,
			ISourceLocation sourceLocation, String reference)
			throws ScannerException {
		if (TapestryCore.isNull(reference))
			return;

		if (reference.startsWith(BindingConstants.OGNL_PREFIX))
			validateExpression(reference.substring(BindingConstants.OGNL_PREFIX
					.length()), severity, sourceLocation);
