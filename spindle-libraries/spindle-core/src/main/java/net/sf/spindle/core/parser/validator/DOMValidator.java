...
/**
 * Validates Tapestry DOM trees. This Validator is tuned to validation Tapestry XML. Its not a
 * generic validator!
 * <p>
 * Uses an IProblemCollector, but does not manage the lifecycle of it. Assumes the provider of the
 * collector will manage it.
 * <p>
...
public class DOMValidator implements IProblemCollector
{
    ...
    private static void parseDTD(String publicId)
    {
        try
        {
            ...
            if (in != null)
                DTDS.put(publicId, new DTDParser(new InputStreamReader(in), publicId, DEBUG)
                        .parse());
            else
                TapestryCore.log(ValidatorMessages.noDTD(publicId));

        }
        catch (IOException e)
        {
            TapestryCore.log(ValidatorMessages.errorParsingDTD(e.getMessage()), e);
        }
        ...
        if (!fSeenIds.isEmpty())
        {
            for (Iterator iter = fSeenIds.entrySet().iterator(); iter.hasNext();)
            {
                ...
                for (Iterator iterator = nodeList.iterator(); iterator.hasNext();)
                {
                    ...
                    recordAttributeError(node, "id", ValidatorMessages.uniqueIDAttribute());
                }
            }
        }
        documentEnd();
    ...
    private boolean canStart()
    {
        ...
        if (fDTD == null)
        {
            reportDocumentError(ValidatorMessages.noDocType(publicId));
            return false;
        }
        return true;
    }
    ...
    private void documentEnd()
    {
        fIsRunning = false;
        if (!fSeenRootElement)
            reportDocumentError(ValidatorMessages.noRootElement());
    }
    ...
    private boolean checkNodeAllowedHere(String name, Node node)
    {
        if (!fSeenRootElement)
        {
            if (!W3CAccess.isElement(node))
            {
                reportDocumentError(ValidatorMessages.invalidRootElement());
                return false;
            }
            ...
            if (!fIsRunning)
                recordTagNameProblem(name, node, ValidatorMessages
                        .wrongRoot(fRootElementName, name));
        ...
        {
            recordTagNameProblem(name, node, ValidatorMessages.undeclaredElement(name));
            fNodeInfoMap.put(node, IGNORE);
            return false;
        }
    ...
    private void checkAttributes(String elementName, Node node)
    {
        ...
        for (Iterator iter = element.attributes.keySet().iterator(); iter.hasNext();)
        {
            ...
            if (sourceAttributeNames.contains(declaredAttrName))
            {
                ...
                if (declaredAttribute.type instanceof DTDEnumeration)
                {
                    ...
                    {
                        recordAttributeError(node, declaredAttrName, ValidatorMessages
                                .invalidAttributeValue(declaredAttrName, declaredAttribute.type
                                        .toString()));
                    }
                }
            ...
            {
                recordErrorOnTagName(node, ValidatorMessages.missingAtrribute(
                        declaredAttrName,
                        elementName));
            }
        }
        if (!sourceAttributeNames.isEmpty())
        {
            for (Iterator iter = sourceAttributeNames.iterator(); iter.hasNext();)
            {
                ...
                recordAttributeError(node, undeclaredName, ValidatorMessages.undeclaredAttribute(
                        undeclaredName,
                        elementName));
            }
        }
    ...
    private void recordErrorOnTagName(Node node, String errorMessage)
    {
        addProblem(
                IProblem.ERROR,
                getNodeSourceInfo(node).getTagNameLocation(),
                errorMessage,
                false,
    ...
    private void recordTagNameProblem(String elementName, Node node, String message)
    {
        addProblem(
                IProblem.ERROR,
                getTagNameLocation(elementName, node),
                message,
                false,
                IProblem.NOT_QUICK_FIXABLE);
    }
    ...
    private static class DTDNodeInfo
    {
        ...
        public void childSeen(Node node) throws ValidatorException
        {
            if (W3CAccess.isTextNode(node))
            {
                if (allowsText)
                    return;
                throw new ValidatorException(ValidatorMessages.textNotAllowed(elementName));
            }
            String childName = node.getNodeName();
            if (workingContent == null)
            {
                throw new ValidatorException(ValidatorMessages.childNotAllowed(
                        childName,
                        elementName));
            }
                    ...
                    || contentType == DTDItemType.DTD_CHOICE)
            {
                ...
                else if (contentType == DTDItemType.DTD_CHOICE)
                {
                    ...
                    {
                        throw new ValidatorException(ValidatorMessages.elementNotAllowed(
                                childName,
                                getAllowedContent()));
                    }
         ...
         * @param childName
         * @param items
         * @return true iff the container is completely satisfied and should be removed from
         *         consideration.
         */
     ...
     * (non-Javadoc)
     * 
     * @see core.source.IProblemCollector#addProblem(int, core.source.ISourceLocation,
     *      java.lang.String)
     */
