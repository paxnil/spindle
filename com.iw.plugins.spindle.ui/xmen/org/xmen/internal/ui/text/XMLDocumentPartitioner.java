      ...
      IDocumentPartitionerExtension2
{
  ...
  protected void initialize()
  {
    ...
    try
    {
      ...
      while (!token.isEOF())
      {
        ...
        {
          //TODO changed here - create XMLNode instead of a TypedPosition
    ...
    try
    {
      ...
      while (!token.isEOF())
      {
        ...
        } else
        {
          ...
          {
            //TODO changed here - create XMLNode instead of a TypedPosition
            d.addPosition(fPositionCategory, new XMLNode(
                start,
                length,
                contentType,
                fDocument));
      ...
      // remove all positions behind lastScannedPosition since there aren't any
      // further types
      if (lastScannedPosition != reparseStart)
      {
        // if this condition is not met, nothing has been scanned because of a
        // deletion
        ++lastScannedPosition;
      }
  ...
  /**
   * Returns the index of the first position which starts at or after the given
   * offset.
   * 
   * @param positions the positions in linear order
   * @param offset the offset
