...
/**
 * Produced by the XMLDocumentPartitioner. Represents an xml artifact in the
 * document.
 * 
...
public class XMLNode extends TypedPosition implements Comparable
{
  ...
  public static synchronized XMLNode createTree(
      String postionCategory,
      IDocument document,
      int stopOffset) throws BadLocationException
  {
    Position[] pos = null;
    try
    {
  ...
  public XMLNode get(int documentOffset)
  {
    if (fPos != null)
    {
      ...
      {
        if (fPos[i].offset <= documentOffset
            && documentOffset < fPos[i].offset + fPos[i].length)
  ...
  public boolean isMultiLine()
  {
    try
    {
      return fDocument.getLineOfOffset(getOffset()) < fDocument
          .getLineOfOffset(getOffset() + getLength());
    } catch (BadLocationException e)
    {
      UIPlugin.log(e);
    }
  ...
  public String getName()
  {
    ...
    } else if (type.equals(XMLDocumentPartitioner.TAG)
    ...
    if (start != -1)
    {
      attrs.add(new XMLNode(
          ...
          content.length() - startLength - start
              - (!getType().equals(XMLDocumentPartitioner.TAG) ? 1 : 0),
          XMLDocumentPartitioner.ATTR,
          fDocument));
    }
    ...
    if (obj instanceof XMLNode)
    {
      ...
      for (Iterator it = artifact.getChildren().iterator(); it.hasNext();)
      {
        ...
        if (childType == XMLDocumentPartitioner.ENDTAG
            || (childType == XMLDocumentPartitioner.TEXT && child
  ...
  //        {
  //            pos =
  // fDocument.getPositions(XMLDocumentPartitioner.CONTENT_TYPES_CATEGORY);
  //        } catch (BadPositionCategoryException e)
  //        {
  //            e.printStackTrace();
  //            return null;
  ...
  /**
   * may be the prev sibling or the parent TODO remove
   */
      ...
      for (int i = index - 1; i >= 0; i--)
      {
        ...
        if (result.getParent() != fParent
            || (type.equals(XMLDocumentPartitioner.TEXT)
                || type.equals(XMLDocumentPartitioner.COMMENT) || type
                .equals(XMLDocumentPartitioner.DECL)))
        {
          continue;
        }
