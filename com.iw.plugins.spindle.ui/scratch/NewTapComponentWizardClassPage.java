...
public class NewTapComponentWizardClassPage extends TapestryWizardPage
{
      ...
      NewTapComponentWizardPage predecessor)
  {
    ...
    this.setImageDescriptor(ImageDescriptor.createFromURL(Images
        .getImageURL("componentDialog.gif")));
    ...
    fMakeAbstractField = new CheckBoxField(
        UIPlugin.getString(MAKE_ABSTRACT + ".label"),
        LABEL_WIDTH,
        true);
  ...
  public void init(IJavaElement jelem)
  {
    ...
    fChooseSpecClassDialogField.init(
        jelem == null ? null : jelem.getJavaProject(),
        context);
  ...
  {
    // 	  unfortunately, this feature (hierarchy root stuff) does not seem to work
    // in Wizards
  ...
  protected void createSpecClassResource(IProgressMonitor monitor) throws InterruptedException,
      CoreException
  {
    ...
    {
      ClassFactory factory = new ClassFactory(false);
      factory.createType(
          ...
          Modifier.PUBLIC | (fMakeAbstractField.getCheckBoxValue() ? Modifier.ABSTRACT : 0),
          true,
          monitor);
      finalSpecClass = factory.getCreatedType();
      fGeneratedJavaFile = (IFile)finalSpecClass.getUnderlyingResource();
    }
  }
