...
public class ProjectInfoDecorator extends AbstractDecorator implements ILightweightLabelDecorator
{
    ...
    private String getDecorationSuffix(IProject project)
    {
        ...
        {
            Object state = TapestryArtifactManager.getTapestryArtifactManager().getLastBuildState(project, false);
            if (state == null)
            {
            ...
            } else
            {
                ...
                INamespace namespace =
                    (INamespace) TapestryArtifactManager.getTapestryArtifactManager().getProjectNamespace(
                        project,
                        false);
                ...
                } else
                {
                    ...
                    if (suffix.length() == 0)
                    {
                        String appname = ((IApplicationSpecification) namespace.getSpecification()).getName();

                        if (appname == null || appname.trim().length() == 0)
                            appname = "unknown";

                        suffix = UIPlugin.getString("project-decorator-standin-specification", appname);
                    }
                }
            }
        }
