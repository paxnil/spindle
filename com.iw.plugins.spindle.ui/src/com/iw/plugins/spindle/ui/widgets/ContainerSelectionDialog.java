...
public class ContainerSelectionDialog extends SelectionDialog
{
   ...
   * @param allowNewContainerName <code>true</code> to enable the user to type
   *                     in a new container name, and <code>false</code> to restrict the
   *                     user to just selecting from existing ones
...
//    WorkbenchHelp.setHelp(shell, IHelpContextIds.CONTAINER_SELECTION_DIALOG);
  ...
  protected Control createDialogArea(Composite parent)
  {
    ...
    Listener listener = new Listener()
    {
      public void handleEvent(Event event)
      {
        if (statusMessage != null && validator != null)
        {
          ...
          {
            statusMessage.setForeground(JFaceColors.getErrorText(statusMessage
                .getDisplay()));
    ...
    group = new ContainerSelectionGroup(
        area,
        listener,
        allowNewContainerName,
        getMessage(),
    ...
    if (fFilter != null) {
      group.getTreeViewer().addFilter(fFilter);
    }
  ...
  public void setFilter(ViewerFilter filter)
  {
    if (group != null)
    {
      TreeViewer viewer = group.getTreeViewer();
      if (fFilter != null)
