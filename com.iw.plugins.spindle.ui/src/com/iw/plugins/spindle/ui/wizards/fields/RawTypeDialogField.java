...
public class RawTypeDialogField extends StringButtonField
{
  ...
  protected IStatus typeChanged()
  {
    ...
    if (!val.isOK())
    {
      ...
      {
        status.setError(UIPlugin.getString(name + ".error.InvalidTypeName", val
            .getMessage()));
        return status;
      } else if (val.getSeverity() == IStatus.WARNING)
      {
        status.setWarning(UIPlugin.getString(name + ".warning.TypeNameDiscouraged", val
            .getMessage()));
      }
    }
    try
        ...
        if (requiredType.isInterface())
        {
          if (!implementsInterface(chosenType, hierarchyRoot))
          {
            status.setError(UIPlugin.getString(
                name + ".error.MustImplementInterface",
                hierarchyRoot));
          } else if (!extendsType(chosenType, requiredType))
          {
            status.setError(UIPlugin.getString(
                name + ".error.MustExtendClass",
                hierarchyRoot));
          }
        }
  ...
  private IType chooseType()
  {
    ...
    try
    {
      ...
      dialog.setTitle(UIPlugin.getString(name + ".TypeDialog.title"));
      String message = UIPlugin.getString(name + ".TypeDialog.message");
