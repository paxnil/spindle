...
public class TypeChooseWizardPage extends NewTypeWizardPage {
	...
	public static String SETTINGS_KEY = UIPlugin.PLUGIN_ID
			+ ".TYPE_CHOOSER_SETTINGS";

	public static String PACKAGE_NAME_KEY = "PACKAGE_NAME";

	public static String PACKAGE_ROOT_KEY = "PACKAGE_ROOT";

	public static String BASE_CLASS_CHOOSER_KEY_PAGE = "BASE_CLASS_PAGE";

	public static String BASE_CLASS_CHOOSER_KEY_COMPONENT = "BASE_CLASS_COMPONENT";

	public static String NEW_CLASS_MODIFIERS_KEY = "NEW_CLASS_MODIFIERS";

	public static String NEW_CLASS_SUPERCLASS_KEY_PAGE = "NEW_CLASS_SUPERCLASS_KEY_PAGE";

	public static String NEW_CLASS_SUPERCLASS_KEY_COMPONENT = "NEW_CLASS_SUPERCLASS_KEY_PAGE";
	 ...
	 * @param pageName
	 */
	public TypeChooseWizardPage(String pageName,
			NewTapComponentWizardPage predecessor, boolean creatingPage) {
	...
	public void init(IJavaElement initElement) {
		...
		String savedRoot = settings.get(PACKAGE_ROOT_KEY);
		String savedPackage = settings.get(PACKAGE_NAME_KEY);

		if (initElement != null) {
			IPackageFragmentRoot root = CoreUtils
					.getPackageFragmentRoot(initElement);
			if (root == null)
				root = getInitialRoot((IJavaProject) CoreUtils
						.findElementOfKind(initElement,
								IJavaElement.JAVA_PROJECT), savedRoot);
			setPackageFragmentRoot(root, true);
			IPackageFragment pack = (IPackageFragment) CoreUtils
					.findElementOfKind(initElement,
							IJavaElement.PACKAGE_FRAGMENT);
			if (pack == null && savedPackage != null)
				pack = root.getPackageFragment(savedPackage);
	...
	private void savePageSettings() {
		...
		if (fCreatingPage) {
			settings.put(BASE_CLASS_CHOOSER_KEY_PAGE,
					fChooseSpecClassDialogField.getTextValue());
			settings.put(NEW_CLASS_SUPERCLASS_KEY_PAGE, getSuperClass());
		} else {
			settings.put(BASE_CLASS_CHOOSER_KEY_COMPONENT,
					fChooseSpecClassDialogField.getTextValue());
			settings.put(NEW_CLASS_SUPERCLASS_KEY_COMPONENT, getSuperClass());
		}
	...
	public void setVisible(boolean visible) {
		...
		if (visible) {
			boolean updateAdvancedClasspath = fFirstPageContainerField
					.getButtonControl(null).isVisible();

			if (updateAdvancedClasspath) {
				setPackageFragmentRoot(fFirstPageContainerField
						.getPackageFragmentRoot(), true);
				setPackageFragment(fFirstPagePackageField.getPackageFragment(),
						true);
			} else {
				updateDefaultContainerAndPackage();
			}
	...
	private void updateDefaultContainerAndPackage() {
		...
		try {
			if (root == null) {
				root = getInitialRoot(fFirstPageProjectField
						.getTapestryProject().getJavaProject(), savedRoot);
			}
		} catch (CoreException e) {
			e.printStackTrace();
		}

		IPackageFragment savedFragment = null;
		if (savedPackage != null) {
			IPackageFragment savedFrag = root.getPackageFragment(savedPackage);
			if (savedFrag != null && savedFrag.exists())
	...
	private IPackageFragmentRoot getInitialRoot(IJavaProject jproject,
		...
		try {
			if (jproject == null)
				return null;
			IPackageFragmentRoot first = null;
			IPackageFragmentRoot saved = null;

			IPackageFragmentRoot[] roots = jproject.getPackageFragmentRoots();
			for (int i = 0; i < roots.length; i++) {
				if (roots[i].getKind() == IPackageFragmentRoot.K_SOURCE) {
					String rname = roots[i].getElementName();
					if (saved == null && rname.equals(savedRoot)) {
						saved = roots[i];
					} else if (first == null) {
						first = roots[i];
					}
	 ...
	 * 
	 * @return the IFile that was generated. If no generation occured, return
	 *         null
	 * @throws JavaModelException
	 *             if there is a problem getting the file from the generated
	 *             Type.
	 */
	...
	public void createControl(Composite parent) {
		...
		int savedModifiers = settings.getInt(NEW_CLASS_MODIFIERS_KEY);
		if (savedModifiers > 0)
			setModifiers(savedModifiers, true);
	...
	private void createClassChoserControls(Composite container, int nColumns) {
		...
		Label specClassLabel = fChooseSpecClassDialogField
				.getLabelControl(container);
		data = new GridData(GridData.HORIZONTAL_ALIGN_FILL);
		data.horizontalSpan = 1;
		specClassLabel.setLayoutData(data);

		Text specClassText = fChooseSpecClassDialogField
				.getTextControl(container);
		data = new GridData();
		...
		new Separator(SWT.NONE).doFillIntoGrid(container, nColumns,
				convertHeightInCharsToPixels(1));

	}

	private void createRadioControls(Composite parent, RadioField field,
			int nColumns) {
		GridData data;
	...
	private void createJavaControls(Composite parent, int nColumns) {
		...
		IDialogSettings settings = getDialogSettings();
		String savedSuperClass = null;
		if (fCreatingPage) {
			savedSuperClass = settings.get(NEW_CLASS_SUPERCLASS_KEY_PAGE);
		} else {
			savedSuperClass = settings.get(NEW_CLASS_SUPERCLASS_KEY_COMPONENT);
		}
		if (savedSuperClass != null) {
			setSuperClass(savedSuperClass, true);
		} else {
		...
		if (fChooseClass.getCheckBoxValue()) {
			...
			if (status.getSeverity() < IStatus.ERROR
				...
				if (!isEnclosingTypeSelected()
					...
					spindle.setError(UIPlugin.getString(PAGE_NAME
							+ ".newclass.package"));
				}
				try {
					...
					// Tapestry
					// one?
					IJavaProject current = getPackageFragmentRoot()
							.getJavaProject();
					if (current != null) {
						if (!current.getProject().hasNature(
								TapestryCore.NATURE_ID)) {
							spindle.setWarning(UIPlugin.getString(PAGE_NAME
									+ ".newclass.project"));

						} else if (!current.equals(fFirstPageProjectField
								.getTapestryProject().getJavaProject())) {
							spindle.setWarning(UIPlugin.getString(PAGE_NAME
									+ ".newclass.project1"));
						}
					}
					...
					boolean hasDefaultSuperclass = fBaseSpecClass
							.equals(getSuperClass())
							|| fDefaultSpecClass.equals(getSuperClass());

					boolean hasDefaultIface = getSuperInterfaces().contains(
							fDefaultInterface);
				...
				status = spindle.getSeverity() >= status.getSeverity() ? spindle
						: status;
			}
	...
	private void checkEnclosingModifiers(SpindleStatus status) {
		...
		if (!(isPublic && isStatic)
				&& fEnclosingTypeStatus.getSeverity() < IStatus.ERROR)
			status.setWarning(UIPlugin.getString(PAGE_NAME
					+ ".newclass.enclosing", getTypeName()));

	}

	/**
	 * Allow subclasses to ensure that if the user has requested that a type be
	 * created that the new type is a valid Tapestry type (in context). Things
	 * we already know:
	 * <ul>
	...
	protected void checkNewType(SpindleStatus status, IJavaProject jproject)
		...
		if (!success)
			status.setError(UIPlugin.getString(PAGE_NAME + ".newclass.error",
					new String[] { getTypeName(), fBaseSpecClass,
							fDefaultInterface }));
	}
	 ...
	 *            the name of an interface the IType might implement instead of
	 *            extending the type
	...
	protected final boolean checkTypes(IType baseType, String tapestryclass,
		...
		try {
			if (!result)
				result = CoreUtils.implementsInterface(baseType,
						tapestryinterface);
		...
		if (project == null) {
			...
			try {
				...
				fChooseSpecClassDialogField.init(jproject, getWizard()
						.getContainer());
				String existingSpecClassname = fChooseSpecClassDialogField
						.getTextValue();
				if (existingSpecClassname == null
						|| existingSpecClassname.trim().length() == 0) {
					IDialogSettings settings = getDialogSettings();
					String saved = null;
					if (fCreatingPage) {
						saved = settings.get(BASE_CLASS_CHOOSER_KEY_PAGE);
					} else {
						saved = settings.get(BASE_CLASS_CHOOSER_KEY_PAGE);
					}

					fChooseSpecClassDialogField
							.setTextValue(saved == null ? fDefaultSpecClass
									: saved);
				} else {
					fChooseSpecClassDialogField
							.setTextValue(existingSpecClassname);
				}
				...
				for (int i = 0; i < roots.length; i++) {
					...
					if (roots[i].getKind() == IPackageFragmentRoot.K_SOURCE
						...
						if (previous != null
								&& !previous.getParent().equals(jproject))
