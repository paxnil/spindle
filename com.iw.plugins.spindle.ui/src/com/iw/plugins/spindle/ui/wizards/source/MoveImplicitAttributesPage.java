...
public class MoveImplicitAttributesPage extends WizardPage
{
  ...
  public MoveImplicitAttributesPage(String pageName)
  {
    ...
    this
        .setDescription("Decide what is moved. Warning: clicking 'finish' is not undoable!");
  ...
  public void createControl(Composite parent)
  {
    ...
    fTemplateComponentId.setTextValue(fInitialTemplateComponentId == null
        ? "" : fInitialTemplateComponentId);
    ...
    AttributesThatStaySection allSection = new AttributesThatStaySection(
        "Attibutes that stay");
    Control allSectionControl = allSection.createControl(leftColumn);
    gd = new GridData(GridData.FILL_BOTH);
    allSectionControl.setLayoutData(gd);
    AttributesThatMoveSection pageSection = new AttributesThatMoveSection(
    ...
    fAttributesThatStayViewer.addDoubleClickListener(new IDoubleClickListener()
    {
      public void doubleClick(DoubleClickEvent event)
      {
        ...
        IStructuredSelection selection = (IStructuredSelection) fAttributesThatStayViewer
            .getSelection();
    ...
    fAttributesThatMoveViewer.addDoubleClickListener(new IDoubleClickListener()
    {
      public void doubleClick(DoubleClickEvent event)
      {
        IStructuredSelection selection = (IStructuredSelection) fAttributesThatMoveViewer
            .getSelection();
        if (!selection.isEmpty())
        {
  ...
  private void updateButtonsEnabled()
  {
    ...
    fRightButton.setEnabled(!fAttributesThatStay.isEmpty()
        && !fAttributesThatStayViewer.getSelection().isEmpty());
    fLeftButton.setEnabled(!fAttributesThatMove.isEmpty()
        && !fAttributesThatMoveViewer.getSelection().isEmpty());
    fUpButton.setEnabled(!fAttributesThatMove.isEmpty()
        && canMoveUp(fAttributesThatMoveViewer.getSelection()));
    fDownButton.setEnabled(!fAttributesThatMove.isEmpty()
        && canMoveDown(fAttributesThatMoveViewer.getSelection()));
  }
  ...
  private boolean canMoveUp(ISelection selection)
  {
    if (selection instanceof IStructuredSelection && !selection.isEmpty())
    {
      ...
      return structured.size() == 1
          && fAttributesThatMove.indexOf(structured.getFirstElement()) > 0;
    }
    return false;
  }
  ...
  private boolean canMoveDown(ISelection selection)
  {
    if (selection instanceof IStructuredSelection && !selection.isEmpty())
    {
      IStructuredSelection structured = (IStructuredSelection) selection;
      return structured.size() == 1
          && fAttributesThatMove.indexOf(structured.getFirstElement()) < fAttributesThatMove
              .size() - 1;
    }
    return false;
  }
  ...
  private void createLeftRightButtons(Composite parent)
  {
    ...
    fRightButton.addSelectionListener(new SelectionListener()
    {
      ...
      public void widgetSelected(SelectionEvent e)
      {
        ...
        handleLeftRightChange(fAttributesThatStayViewer, fAttributesThatStayViewer
            .getSelection());
      }
    ...
    fLeftButton.addSelectionListener(new SelectionListener()
    {
      ...
      public void widgetSelected(SelectionEvent e)
      {
        ...
        handleLeftRightChange(fAttributesThatMoveViewer, fAttributesThatMoveViewer
            .getSelection());
      }
