...
public class XMLFormattingStrategy implements XMLContentFormatter.FormattingStrategy
{
  ...
  private static class LineWalkerX
  {
    ...
    public LineWalkerX(IDocument document, IRegion region, String delimiter,
        int[] positions) throws BadLocationException
    {
  ...
  /*
   * run variables reset for every invocation. Defined as state variables so we
   * dont need to pass them around all the time. Note that this makes the
   ...
   * (non-Javadoc)
   * 
   * @see com.iw.plugins.spindle.editors.XMLContentFormatter.FormattingStrategy#format(org.eclipse.jface.text.IDocument,
   *      int, int, int[])
   */
  ...
  /**
   * compute the array of DocumentArtifacts. sorted by offset.
   */
  private void computeTypedPositions()
  {
    ...
    try
    {
      Position[] positions = fDocument
          .getPositions(XMLDocumentPartitioner.CONTENT_TYPES_CATEGORY);
   ...
   * Terminate with a line delimiter.
   * 
  ...
  /**
   * @return whether empty lines should be preserved during formatting (default:
   *         true)
   */
  ...
  /**
   * @return whether indenting should be done by tabs (default: true). This will
   *         also replace any existing space indents.
   */
  ...
  /**
   * Determine whether indenting should be done by tabs (default: true). This
   * will also replace any existing space indents.
   * 
