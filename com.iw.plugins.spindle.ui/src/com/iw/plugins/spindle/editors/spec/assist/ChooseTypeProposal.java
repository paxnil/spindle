...
public class ChooseTypeProposal implements ICompletionProposal
{
    ...
    public ChooseTypeProposal(IJavaProject project, boolean includeInterfaces, int documentOffset,
            int replacementOffset, int replacementLength)
    {
        this(project, null, includeInterfaces, documentOffset, replacementLength, replacementLength);
    }
    ...
    protected String chooseType(String title)
    {
        ...
        try
        {
            ...
            int style = 0;
            if (fHierarchyRoot == null)
            {
                if (fIncludeInterfaces)
                    style = IJavaElementSearchConstants.CONSIDER_ALL_TYPES;
                else
                    style = IJavaElementSearchConstants.CONSIDER_CLASSES_AND_ENUMS;
            }
            else
            {
                if (fIncludeInterfaces)
                    ...
                    style = IJavaElementSearchConstants.CONSIDER_CLASSES;
            }
            SelectionDialog dialog = JavaUI.createTypeDialog(shell, PlatformUI.getWorkbench()
                    .getProgressService(), scope, style, false);
    ...
    protected IJavaSearchScope createSearchScope(IJavaProject jproject) throws JavaModelException
    {
        IJavaElement[] elements = new IJavaElement[]
        { jproject };
    ...
    protected IType resolveTypeName(IJavaProject jproject, String typeName)
            throws JavaModelException
    {
        if (jproject == null)
            return null;
