...
public class DefaultCompletionProcessor extends SpecCompletionProcessor
{
    ...
    protected ICompletionProposal[] doComputeCompletionProposals(ITextViewer viewer, int documentOffset)
    {
        ...
        node = nextNode;
        //we know its a text artifact - lets see if the user is trying to insert the root tag!
    ...
    private void computeAdditionalProposals(ITextViewer viewer, int documentOffset, List proposals)
    {
        ...
        proposals.add(CommentCompletionProcessor.getDefaultInsertCommentProposal(documentOffset, 0));
    }

    protected ICompletionProposal[] computeRootTagProposal(XMLNode currentNode, XMLNode rootNode, int documentOffset)
    {
        ...
        {
            proposals = DTDProposalGenerator.getNewElementCompletionProposals(fDTD, "component-specification");
        } else if (name.endsWith(".page"))
        {
            proposals = DTDProposalGenerator.getNewElementCompletionProposals(fDTD, "page-specification");
        } else if (name.endsWith(".application"))
        {
            proposals = DTDProposalGenerator.getNewElementCompletionProposals(fDTD, "application");
        } else if (name.endsWith(".library"))
        {
            proposals = DTDProposalGenerator.getNewElementCompletionProposals(fDTD, "library-specification");
        }
