 ...
 *                     Exp $
 */
public class XMLDocumentFormatter implements XMLContentFormatter.FormattingStrategy
{
  /**
  ...
  private FormattingPreferences fPreferences;
  private int fOffset;
      ...
      int[] positions)
  {
    ...
    try
    {
      ...
      reconciler.createTree(document);
      fRoot = reconciler.getRoot();
      List stored = reconciler.getStoredPos();
      fDocumentPositions = (TypedPosition[]) stored.toArray(new TypedPosition[stored
          .size()]);
  ...
  private String doFormat(int length) throws BadLocationException
  {
    ...
    while (tposition != null)
    {
      ...
      {
        formatDefault(tposition, buffer);
      } else if (type == ITypeConstants.TEXT || type == ITypeConstants.PI)
      {
        formatDefault(tposition, buffer);
      }
