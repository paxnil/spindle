 ...
 * @version $Revision$
 */
public class XMLContentFormatterChristian implements IContentFormatter 
{
	...
	static class PositionReference implements Comparable {
		 ...
		 */
		protected PositionReference(
			Position position, boolean refersToOffset, String category, boolean isDuplicate) 
		{
			fPosition= position;
		...
		public void setCharacterPosition(int position) {
			if (refersToOffset())
				...
				setLength(position - getOffset() + 1);
		}
	 ...
	 */
	public XMLContentFormatterChristian(FormattingStrategy formattingStrategy, String[] partitioningCategories)
	{
	...
	private void updateAffectedPositions(IDocument document, int[] positions, int offset) {
		...
		for (int i= 0; i < positions.length; i++) {
			...
			if (!r.isDuplicate()) { //document.containsPosition(category, p.offset, p.length)) { WHOA!!
				try {
