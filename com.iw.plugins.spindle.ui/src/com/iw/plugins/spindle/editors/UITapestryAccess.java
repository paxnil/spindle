...
/**
 * Access Tapestry Artifacts for various UI tasks. public methods are not meant to be resued. Create
 * one when you need it, or just call the static methods.
 * 
...
public abstract class UITapestryAccess
{
    ...
    public static Result[] getAllChildNamespaceComponents(ICoreNamespace namespace)
    {
        ...
        for (Iterator iter = libIds.iterator(); iter.hasNext();)
        {
            ...
            for (Iterator iter2 = types.iterator(); iter2.hasNext();)
            {
                ...
                result.add(createComponentInformationResult(libId + ":" + typeId, typeId
                        + " - from '" + libId + "'", component));
            }
        }
    ...
    public static Result[] getChildNamespaceComponents(ICoreNamespace namespace, String libraryId)
    {
        ...
        for (Iterator iter = types.iterator(); iter.hasNext();)
        {
            ...
            result
                    .add(UITapestryAccess.createComponentInformationResult(
                            typeId,
                            typeId,
                            component));
        }
    ...
    public static UITapestryAccess.Result[] getLibraryIds(INamespace namespace)
    {
        ...
        for (Iterator iter = libIds.iterator(); iter.hasNext();)
        {
            ...
            result.add(new UITapestryAccess.Result(libId,
                    description == null ? "no description available" : description));
        }
        return (UITapestryAccess.Result[]) result
                .toArray(new UITapestryAccess.Result[result.size()]);
    }

    private static Result createChildNamespaceResult(String libId,
            ILibrarySpecification namespaceSpec)
    {
    ...
    public static Result[] getComponents(ICoreNamespace framework, ICoreNamespace currentNamespace)
    {
        ...
        for (Iterator iter = types.iterator(); iter.hasNext();)
        {
            ...
            applicationTypes.put(
                    typeId,
                    createComponentInformationResult(typeId, typeId, component));
        }
        ...
        if (framework != null)
        {
            ...
            for (Iterator iter = types.iterator(); iter.hasNext();)
            {
                ...
                applicationTypes.put(typeId, createComponentInformationResult(
                        typeId,
                        typeId,
                        component));
            }
    ...
    public static Result[] findParameters(PluginComponentSpecification component, String match,
            Set existing)
    {
        if (component == null)
            return new Result[] {};
        ...
        for (Iterator iter = parameterMap.keySet().iterator(); iter.hasNext();)
        {
            ...
            IParameterSpecification parameterSpec = (IParameterSpecification) parameterMap
                    .get(name);
            ...
            result.add(UITapestryAccess.createParameterResult(name, parameterSpec, component
                    .getPublicId()));
        }
    ...
    public static Result createComponentInformationResult(String name, String displayName,
    ...
    public static Result createParameterResult(IComponentSpecification component,
    ...
    public static Result createParameterResult(String name, IParameterSpecification parameterSpec,
            String publicId)
    {
    ...
    public UITapestryAccess(Editor editor) throws IllegalArgumentException
    {
        ...
        Assert.isLegal(fNamespace != null, "can't determine the namespace for "
                + editor.getEditorInput().getName());

        IResourceWorkspaceLocation loc = (IResourceWorkspaceLocation) fNamespace
                .getSpecificationLocation();

        IProject project = loc.getProject();

        TapestryArtifactManager tapestryArtifactManager = TapestryArtifactManager
                .getTapestryArtifactManager();
        fFrameworkNamespace = (ICoreNamespace) tapestryArtifactManager
                .getFrameworkNamespace(project);

        try
        {
            if (fFrameworkNamespace == null && !project.hasNature(TapestryCore.NATURE_ID))
            {
                IStorage nsStorage = loc.getStorage();
                IProject[] potentials = Markers.getHomeProjects(project);
                for (int i = 0; i < potentials.length; i++)
                {
                    Map specMap = tapestryArtifactManager.getSpecMap(potentials[i]);
                    if (specMap != null && specMap.containsKey(nsStorage))
                    {
                        fFrameworkNamespace = (ICoreNamespace) tapestryArtifactManager
                                .getFrameworkNamespace(potentials[i]);
                        break;
                    }
                }
            }
