...
public abstract class UITapestryAccess
{
  ...
  public static Result[] getChildNamespaceComponents(
      ...
      String libraryId)
  {
    ...
    for (Iterator iter = types.iterator(); iter.hasNext();)
    {
      ...
      result.add(UITapestryAccess.createComponentInformationResult(
          typeId,
          typeId,
          component));
    }
  ...
  public static UITapestryAccess.Result[] getLibraryIds(INamespace namespace)
  {
    ...
    for (Iterator iter = libIds.iterator(); iter.hasNext();)
    {
      ...
      result.add(new UITapestryAccess.Result(libId, description == null
          ? "no description available" : description));
    }
    return (UITapestryAccess.Result[]) result.toArray(new UITapestryAccess.Result[result
        .size()]);
  }

  private static Result createChildNamespaceResult(
      String libId,
      ILibrarySpecification namespaceSpec)
  {
    ...
    pwriter.println();
    XMLUtil.writeLibrary(
        libId,
        namespaceSpec.getLibrarySpecificationPath(libId),
        pwriter,
        0);
  ...
  public static Result[] getComponents(
      ...
      ICoreNamespace currentNamespace)
  {
    List result = new ArrayList();
    ...
    {
      String typeId = (String) iter.next();
      IComponentSpecification component = currentNamespace
          .getComponentSpecification(typeId);
  ...
  public static Result[] findParameters(
  ...
  public static Result createParameterResult(
