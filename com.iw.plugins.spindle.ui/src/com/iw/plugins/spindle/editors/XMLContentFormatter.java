...
public class XMLContentFormatter implements IContentFormatter
{
    ...
    {
        /**
         * Do the actual formatting. The document should not be modified by this method. Instead, 
         * the formatted string must be returned and will be used by the caller to replace the 
         * document text, after synchronizing positionings.
         * 
         ...
         * @return the formatted string to be inserted in place of the selected region
         */
        ...
        /**
         * Determine whether indenting should be done by tabs.
         * This will also replace any existing indents.
         * 
    ...
    static class PositionReference implements Comparable
    {
         ...
         * @param isDuplicate whether this is a duplicate reference, if both offset and length overlap
         */
        protected PositionReference(Position position, boolean refersToOffset, String category, boolean isDuplicate)
        {
            fPosition = position;
         ...
         * 
         * a check is done to ensure that a zero length position returns
         * its offset as the character position for its length.
         * 
    ...
    /**
     * @param partitioningCategories the position categories which are used to manage 
     * the document's partitioning information and thus should be ignored when this 
     * formatter updates positions
     */
    public XMLContentFormatter(FormattingStrategy formattingStrategy, String[] partitioningCategories, IPreferenceStore store)
    {
    ...
    public void format(IDocument document, IRegion region)
    {
        try
        {
            ...
            fFormattingStrategy.setPreserveEmpty(fPreferenceStore.getBoolean(PreferenceConstants.FORMATTER_PRESERVE_BLANK_LINES));
            fFormattingStrategy.setUseTabIndent(fPreferenceStore.getBoolean(PreferenceConstants.FORMATTER_USE_TABS_TO_INDENT));
    ...
    /*
     * We dont support/use formatting strategies, therefore this method returns <code>null</code>
                ...
                try
                {
                    ...
                    for (int j = 0; j < positions.length; j++)
                    {
                        ...
                        if (p.overlapsWith(offset, length))
                        {
                            ...
                            {
                                duplicate = true;
                                fOverlappingPositionReferences.add(
                                    new PositionReference(p, true, categories[i], false));
                            }
                            if (p.getOffset() + p.getLength() < offset + length)
                                fOverlappingPositionReferences.add(
                                    new PositionReference(p, false, categories[i], duplicate));
                        }
                    }
    ...
    private void updateAffectedPositions(IDocument document, int[] positions, int offset)
    {
        ...
        if (positions.length == 0)
            return;
        int count = 0;
        // things get wonky because its (the old way) not guranteed that the offset of positions is
        // updated first. If the offset is not updated first, then the length *is*.
        // And the length will  always be wrong if the offset moved and has not been updated yet.
