...
public class Utils {
			...
			for (int x = 0; x < pages.length; x++) {
				...
				for (int j = 0; j < erefs.length; j++) {
					...
					if (iEditorReference.isDirty()) {
						result.add(iEditorReference.getEditor(false));
					}					
				}
			}
		}
		...
		for (int i = 0; i < windows.length; i++) {
			...
			for (int x = 0; x < pages.length; x++) {
				...
				for (int z = 0; z < editors.length; z++) {
					...
					ITapestryModel editorModel =
						TapestryPlugin.getTapestryModelManager().getModel(
							editorFile);
	...
	public static List getApplicationsWithAlias(String alias) {
		...
		Iterator iter =
			TapestryPlugin.getTapestryModelManager().applicationsIterator();
		while (iter.hasNext()) {
	...
	public static void copyContainedComponentTo(
		...
		PluginContainedComponent resultComponent =
			new PluginContainedComponent();
		...
		while (iter1.hasNext()) {
			...
			PluginBindingSpecification binding =
				(PluginBindingSpecification) sourceComponent.getBinding(
					parameter);
			PluginBindingSpecification bindingCopy =
				new PluginBindingSpecification(
					binding.getType(),
					binding.getValue());
	 ...
	 */
	public static IJavaElement findElementOfKind(
	 ...
	 */
	public static boolean isOnBuildPath(
	...
	public static IType findType(
		...
		if (elem instanceof ICompilationUnit) {
			return findTypeInCompilationUnit(
				(ICompilationUnit) elem,
				typeQualifiedName);
	 ...
	 * @return the type found, or null if not existing
	 */
	public static IType findTypeInCompilationUnit(
		ICompilationUnit cu,
		String typeQualifiedName)
		throws JavaModelException {
	...
	*/
	...
	public static boolean isVisible(IMember member, IPackageFragment pack)
		...
		IPackageFragment otherpack =
			(IPackageFragment) findElementOfKind(member,
	...
	public static boolean extendsType(IType candidate, IType baseType)
		throws JavaModelException {
		boolean match = false;
		...
		if (superInterfaces != null && superInterfaces.length > 0) {
			for (int i = 0; i < superInterfaces.length; i++) {
				if (candidate.isBinary()
					&& interfaceName.endsWith(superInterfaces[i])) {
					match = true;
				} else {
