  ...
  protected IStatus nameChanged() {
    ...
    if (!val.isOK()) {
      if (val.getSeverity() == IStatus.ERROR) {
        status.setError(MessageUtil.getFormattedString(fieldName + ".error.InvalidComponentName", val.getMessage()));
        return status;
      } else if (val.getSeverity() == IStatus.WARNING) {
        status.setWarning(
          MessageUtil.getFormattedString(fieldName + ".warning.ComponentNameDiscouraged", val.getMessage()));
        return status;
      }
    }
    if (packageField != null && packageField.getPackageFragment() != null) {
      try {
        ...
        IFile file = folder.getFile(new Path(appname + ".jwc"));
        if (file.exists()) {
          status.setError(MessageUtil.getFormattedString(fieldName + ".error.ComponentAlreadyExists", appname));
        }
    ...
    if (Character.isLowerCase(first)) {
      status.setWarning(
        MessageUtil.getFormattedString(
          fieldName + ".warning.ComponentNameDiscouraged",
