...
public class I18NDocumentProvider extends FileDocumentProvider {
	...
	/*
	 * @see org.eclipse.ui.editors.text.StorageDocumentProvider#setDocumentContent(IDocument, InputStream, String)
	 */
	protected void setDocumentContent(
		IDocument document, InputStream contentStream, String encoding
		...
		try {
			...
			IStatus s = new Status(IStatus.ERROR,
				PlatformUI.PLUGIN_ID, IStatus.OK, msg, x);
	...
	protected void doSaveDocument(
		IProgressMonitor monitor, Object element,
		...
		try {
			...
			String encoding = getDeclaredEncoding(
			...
			try {
				...
				IStatus s = new Status(IStatus.ERROR,
					...
					EditorMessages.getString(
						"I18NDocumentProvider.error.encoding"), e);
			...
			if (file.exists()) {
				...
				try {
					monitor.beginTask(
						EditorMessages.getString(
							"I18NDocumentProvider.task.saving"), //$NON-NLS-1$
							2000 );
			...
			IStatus s = new Status(IStatus.ERROR,
				PlatformUI.PLUGIN_ID, IStatus.OK, x.getMessage(), x);
