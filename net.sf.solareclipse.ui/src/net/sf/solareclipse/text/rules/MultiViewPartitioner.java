	...
	protected void addInnerRegion(FlatNode position) {
		if (outerDocument != null) {
			DocumentEvent event = new DocumentEvent(outerDocument,
				outerDocument.getLocalOffset(position.offset), position.length, null);
		...
		if (position instanceof ViewNode) {
			...
			if (partitioner != null) {
				InnerDocumentView innerDocument =
					new InnerDocumentView(document, (ViewNode) position);
...
//		outerDocument.addDocumentPartitioningListener(viewListener);
		...
		if (first > 0) {
			...
			if (offset < right) {
				...
				if (innerDocument != null) {
					...
					int start = innerDocument.getLocalOffset(offset);
					innerDocument.fireDocumentAboutToBeChanged(
						new DocumentEvent(innerDocument, start,
							innerDocument.getLength() - start, null));
				}
			}
		}
				...
				if (end < right) {
					...
					if (p instanceof ViewNode) {
						...
						if (innerDocument != null) {
							...
							innerDocument.fireDocumentChanged(
								new DocumentEvent(innerDocument, start, length, text));
						}
					}
				} else {
					// cut partition at right
					...
					if (p instanceof ViewNode) {
						...
						if (innerDocument != null) {
							...
							innerDocument.fireDocumentChanged(
								new DocumentEvent(innerDocument, start, cut, null));
						}
					}
				}
			}
	...
	private void addOuterPartitions(
		List list, int offset, int length, FlatNode prev, FlatNode next
	) {
		// limit region
		int start = offset;
		int end = offset + length;
		...
		if (outerDocument == null) {
			list.add(new TypedRegion(start, end - start,
				getContentType(null, IDocument.DEFAULT_CONTENT_TYPE)));
			return;
		}
		...
		try {
			...
			ITypedRegion[] regions = outerDocument
				.computePartitioning(start, end - start);
	...
	private void addInnerPartitions(
		List list, int offset, int length, FlatNode position
	) {
		InnerDocumentView innerDocument = null;
		if (position instanceof ViewNode) {
			// TODO: revisit condition
		...
		try {
			...
			ITypedRegion[] regions = innerDocument
	...
	protected ITypedRegion getOuterPartition(
		int offset, FlatNode prev, FlatNode next
	) {
		try {
			int start, end; String type;

			if (outerDocument == null) {
				start = 0; end = document.getLength();
				type = getContentType(null, IDocument.DEFAULT_CONTENT_TYPE);
			} else {
				ITypedRegion region = outerDocument
					.getPartition(outerDocument.getLocalOffset(offset));
